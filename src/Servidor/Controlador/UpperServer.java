			...
			try {
				...
				if (casoEsperandoJugador && casoSolicitudNuevoJugador) {
					...
					if (jugadores.size() == 0) {
						...
						respuesta = clientSentence.toUpperCase() + " conectado.";
					} else {
						boolean jugadorExistente = jugadores.get(0).getNombreJugador()
								.compareTo(clientSentence.toUpperCase()) == 0;
						if (jugadorExistente == false) {
							jugadores.add(new Jugador(clientSentence.toUpperCase()));
							respuesta = clientSentence.toUpperCase() + " conectado.";
							seleccionDeTurnos = true;
						} else {
							System.out.println(jugadores.get(0).getNombreJugador() + " ya se encontraba conectado.");
							respuesta = jugadores.get(0).getNombreJugador() + " ya se encontraba conectado.";
						}
					}
				...
				if (seleccionSimbolo == true) {
					...
					if (oracionEnMayusculas.contains(jugadores.get(0).getNombreJugador())) {
						jugadores.get(0)
								.setSimboloJugador("" + oracionEnMayusculas.charAt(oracionEnMayusculas.length() - 1));
				...
				if (manejoDeJuego == true) {
					...
					Movimiento nuevoMovimiento = new Movimiento(Integer.parseInt(partesMovimientoRealizado[0]),
							Integer.parseInt(partesMovimientoRealizado[1]), jugadorActual);
